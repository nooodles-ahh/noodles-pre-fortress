...
unsigned int CTFGrenadePipebombProjectile::PhysicsSolidMaskForEntity( void ) const
{
	return BaseClass::BaseClass::PhysicsSolidMaskForEntity();
}
...
void CTFGrenadePipebombProjectile::PipebombTouch( CBaseEntity *pOther )
{
	...
	if (m_iType == TF_GL_MODE_REGULAR && 
		pOther->GetTeamNumber() != 0 && (pOther->GetTeamNumber() != GetTeamNumber() || TFGameRules()->FriendlyFireMode()) &&
		pOther->m_takedamage != DAMAGE_NO)
	{
		...
		m_hEnemy = pOther;
		m_bTouchedWallDirectHitOnPlayer = m_bTouchedWall;
...
void CTFGrenadePipebombProjectile::VPhysicsCollision( int index, gamevcollisionevent_t *pEvent )
{
	...
	{
		// Blow up if we hit an enemy we can damage
		if( pHitEntity->GetTeamNumber() != 0 && (pHitEntity->GetTeamNumber() != GetTeamNumber() || TFGameRules()->FriendlyFireMode()) &&
			pHitEntity->m_takedamage != DAMAGE_NO )
		{
			// Save this entity as enemy, they will take 100% damage.
			m_hEnemy = pHitEntity;
		...
		{
			m_bTouched = true;
			m_bTouchedWall = false;
		}
		else
		{
			m_bTouchedWall = true;
			//DevMsg( "we have touched a wall\n" );
		}
	}
	...
	if (pInflictor)
	{
		...
		if ( (!bSameTeam && !pGrenade) || (!bSameTeam && pGrenade && pGrenade->IsPillGrenade()) )
		{
			...
			if ( info.GetDamageType() != DMG_BLAST 
				&& info.GetDamageType() != DMG_BLAST_SURFACE 
				&& pWeapon && pWeapon->GetWeaponID() != TF_WEAPON_FLAMETHROWER_ROCKET 
				&& pWeapon->GetWeaponID() != TF_WEAPON_ROCKETLAUNCHER
				&& m_iType == TF_GL_MODE_REMOTE_DETONATE )
			{
				Fizzle();
			}
