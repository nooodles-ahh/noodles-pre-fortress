...
class CTFWeaponBase : public CBaseCombatWeapon
{
...
#ifdef PF2
	PFLoadoutWeaponInfo *GetPFItemInfo();
#endif
	...
	static acttable_t m_acttableItem3[];
	static acttable_t m_acttableItem4[];
...
#ifdef PF2
	virtual const char *GetMuzzleFlashModel_1st(void);
	...
	virtual const char *GetMuzzleFlashParticleEffect_1st( void );

#ifdef PF2_CLIENT
	virtual int GetMuzzleModelAttachment( void ) { return m_iMuzzleModelAttachment; }
#endif
...
#ifdef PF2_CLIENT
	int m_iMuzzleModelAttachment; // cache the muzzle_model attachment for use for not formatting it for the viewmodel
#endif
private:
