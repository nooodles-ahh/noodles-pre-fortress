...
CTFBaseProjectile *CTFBaseProjectile::Create( const char *pszClassname, const Vector &vecOrigin, 
											...
											CBaseEntity *pScorer, bool bCritical, float flGravity )
{
	...
	pProjectile->SetGravity(flGravity);
...
// Purpose: 
//-----------------------------------------------------------------------------
C_LocalTempEntity *ClientsideProjectileCallback( const CEffectData &data, const char *pszParticleName )
{
	...
	float flGravity = ( data.m_flMagnitude * 800 );
...
void CTFBaseProjectile::ProjectileTouch( CBaseEntity *pOther )
{
	...
	if( pOther->IsPlayer() )
	{
		IGameEvent *event = gameeventmanager->CreateEvent( "projectile_impact", true );
		if( event )
		{
			event->SetInt( "attachedEntity", pOther->entindex() );
			event->SetInt( "shooter", pOwner ? pOwner->entindex() : 0 );
			event->SetInt( "attachedEntity", pOther->entindex() );
			...
			event->SetInt( "projectileType", GetWeaponID() );
			DevMsg( "projectiles weapon id = %d", event->GetInt( "projectileType", 0 ) );
