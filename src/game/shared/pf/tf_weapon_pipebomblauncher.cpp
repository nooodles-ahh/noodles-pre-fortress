...
bool CTFPipebombLauncher::Deploy( void )
{
	...
	if( m_flDetSoundDuration <= 0.0f )
	{
		...
		m_flDetSoundDuration = enginesound->GetSoundDuration( wav );
	}
...
void CTFPipebombLauncher::SecondaryAttack( void )
{
	...
	if ( m_iPipebombCount )
	{
		...
		else
		{
			...
			{
				m_flNextDetSoundTime = gpGlobals->curtime + m_flDetSoundDuration;
