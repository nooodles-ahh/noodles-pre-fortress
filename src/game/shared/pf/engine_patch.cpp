...
#ifdef CLIENT_DLL
    // mat_picmip uncap
    // don't know why this has to be split up like this
    //max
    {
        "PicmipUncap",
        "\x74\x31\xC7\x44\x24\x08\x02\x00\x00\x00\xC7\x44\x24\x04\xFF\xFF\xFF\xFF",
        "xxxxxx????xxxx????",
        6,
        PATCH_IMMEDIATE,
        10,
...
void CEnginePatch::Constructor( const char* name, char* signature, char* mask, size_t offset, bool immediate )
{
	m_sName = name;
...
{
	Constructor( name, signature, mask, offset, immediate );
}
