...
void CWeaponMedigun::WeaponReset( void )
{
	...
	m_flNextSoundStop = 0.0f;
#else
	m_bHealingSelf = false;
#endif

	m_PrevChargeTargets.RemoveAll();
	// Dunno
	m_PrevChargeTargets.EnsureCapacity( 16 );
		...
		if ( !m_bChargeRelease )
		{
			if ( pTFPlayer )
			{
				if ( weapon_medigun_charge_rate.GetFloat() )
				{
					...
					bool bSetupChargeRate = TFGameRules() && TFGameRules()->InSetup() && TFGameRules()->GetActiveRoundTimer();
					// Reduced charge for healing fully healed guys
					if ( pNewTarget->GetHealth() >= iBoostMax && !bSetupChargeRate )
					{
						flChargeAmount *= 0.5;
					}
					else if ( bSetupChargeRate )
					{
						flChargeAmount *= 2.0f;
					}
#endif
...
void CWeaponMedigun::DrainCharge( void )
{
	...
	if ( m_bChargeRelease )
	{
		...
		// Cyanide; Read https://www.teamfortress.tv/3374/details-on-uber-splitting#3
		// 1 previous target = 150%, 2 = 200%, 3 = 250%, 4 = 300%, etc
		// So to get that we have the existing amount to drain for this frame + half of this amount
		// for each player that currently has uber wearing off
		float flDrainPenalty = flChargeAmount * 0.5f;

		FOR_EACH_VEC( m_PrevChargeTargets, i )
		{
			CTFPlayer *pTFPlayer = ToTFPlayer( m_PrevChargeTargets[i] );
			// is this previous target an actual drain?
			if( pTFPlayer && pTFPlayer->IsAlive() && pTFPlayer->m_Shared.InCond(TF_COND_INVULNERABLE_WEARINGOFF))
			{
				flChargeAmount += flDrainPenalty;
			}
			else
			{
				// remove then decrement
				m_PrevChargeTargets.Remove( i-- );
				continue;
			}
		}
		...
		if ( !m_flChargeLevel )
		{
			...
			pOwner->m_Shared.RecalculateInvuln();
#endif
			// Remove all prev charge targets
			m_PrevChargeTargets.RemoveAll();
		}
	}
}
...
void CWeaponMedigun::ItemPostFrame( void )
{
	...
	if ( pOwner->GetMedigunAutoHeal() )
	{
		...
		{
			if ( m_bCanChangeTarget && m_hHealingTarget )
			{
				RemoveHealingTarget();
#if defined( CLIENT_DLL )
				// horrible, lazy even, but hearing is a powerful sense
				if( m_flNextSoundStop < gpGlobals->curtime )
				{
					m_bPlayingSound = false;
					StopHealSound(true, false);
					m_flNextSoundStop = gpGlobals->curtime + 0.07;
				}
...
void CWeaponMedigun::RemoveHealingTarget( bool bStopHealingSelf )
{
	...
	// are we draining charge
	if( m_bChargeRelease && m_hHealingTarget && m_hHealingTarget->IsAlive() )
	{
		bool bFound = false;
		//check if this target is already on the list of previous targets
		FOR_EACH_VEC( m_PrevChargeTargets, i )
		{
			if( m_PrevChargeTargets[i] == nullptr )
			{
				// remove then decrement
				m_PrevChargeTargets.Remove( i-- );
				continue;
			}
			if( m_PrevChargeTargets[i] == m_hHealingTarget.Get() )
			{
				bFound = true;
				break;
			}
		...
		if( !bFound )
		{
			m_PrevChargeTargets.AddToTail( m_hHealingTarget.Get() );
		}
	}
