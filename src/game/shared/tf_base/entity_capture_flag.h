...
#define TF_FLAG_THINK_TIME			0.5f
...
enum NeutralType
{
	TF_INVADE_RETURN_NONE = 0,
	TF_INVADE_RETURN_DEFAULT,
	TF_INVADE_RETURN_HALF
};
...
#ifdef PF2
	virtual void	Drop( CTFPlayer *pPlayer, bool bVisible, bool bThrown = false, bool bMessage = true, bool bThrowingFlag = false );
	void			Throw( CTFPlayer *pPlayer );

	bool			SlowCarrier( void ) { return m_bSlowPlayer; }
	unsigned int	PhysicsSolidMaskForEntity( void ) const { return MASK_PLAYERSOLID; }
	void			PhysicsToggle( bool bToggle );
#else
...
#ifdef PF2
	CNetworkVar( bool, m_bSlowPlayer );
	string_t m_FlagModelName;
	Vector m_VecFlagRedHome;
	Vector m_VecFlagBlueHome;
#endif
	int				m_iOriginalTeam;
...
#ifdef PF2_DLL
	COutputEvent	m_outputOnThrow;	// Fired when the flag is thrown.	
	COutputEvent	m_outputOnTouchOpposingTeam;
#endif
	//Live outputs
...
#ifdef PF2_DLL
	bool			m_bWasThrown;
	bool			m_bWasReturnedByOpposingTeam;
#endif
	
	int				m_nReturnTime;
	int 			m_nNeutralType;
	bool			m_bInvadeScoringType;
#else
