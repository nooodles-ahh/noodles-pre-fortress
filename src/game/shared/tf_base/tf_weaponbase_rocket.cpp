...
#ifdef PF2_DLL
	if( TFGameRules()->FriendlyFireMode() > FFMODE_OFF )
	{
...
CTFBaseRocket *CTFBaseRocket::Create( const char *pszClassname, const Vector &vecOrigin, 
									  const QAngle &vecAngles, CBaseEntity *pOwner, float flVelocity )
#else
CTFBaseRocket *CTFBaseRocket::Create( const char *pszClassname, const Vector &vecOrigin, 
...
#ifdef PF2_DLL
void CTFBaseRocket::SetGravity(float flGravity)
{
	BaseClass::SetGravity( flGravity );
	if(flGravity != 0.0f)
