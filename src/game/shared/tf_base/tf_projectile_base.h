...
C_LocalTempEntity *ClientsideProjectileCallback( const CEffectData &data, const char *pszParticleName = NULL );
#endif
...
protected:
	...
	static CTFBaseProjectile *Create( const char *pszClassname, const Vector &vecOrigin, 
		const QAngle &vecAngles, CBaseEntity *pOwner, float flVelocity, short iProjModelIndex, const char 
		*pszDispatchEffect = NULL, CBaseEntity *pScorer = NULL, bool bCritical = false, float flGravity = 0.0f );
