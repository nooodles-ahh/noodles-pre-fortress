...
public:
	static CBasePlayer* ClientPutInServerOverride_Bot( edict_t* pEdict, const char* playername )
	{
		...
		engine->SetFakeClientConVarValue( pEdict, "pf_sniper_toggle_charge", "0" );
		engine->SetFakeClientConVarValue( pEdict, "cl_flipviewmodels", "0" );
...
void cc_bot_kick( const CCommand &args )
{
	...
	char const *pVal = args.FindArg( "-except" );
	...
	else if( pVal )
	{
		...
		CTFPlayer *pPlayer = ToTFPlayer( UTIL_PlayerByName( pVal ) );
		...
		for( int i = 1; i <= gpGlobals->maxClients; i++ )
		{
			...
			if( pPlayer && (pPlayer->GetFlags() & FL_FAKECLIENT) )
			{
				if( Q_strcmp( pVal, pPlayer->GetPlayerName() ) != 0 )
				{
					engine->ServerCommand( CFmtStr( "kickid %d %s;", pPlayer->GetUserID(), "Kicked by server." ) );
				}
			}
...
static ConCommand bot_kick( "bot_kick", cc_bot_kick, "kick all bots or kick a single bot \n -except <bot name> kick everyone except this bot (case sensitive)", FCVAR_CHEAT);
