...
public:
	...
	virtual void Explode( CBaseEntity *pOwner );

	virtual unsigned int PhysicsSolidMaskForEntity( void ) const;

	int GiveAmmo( int iCount, int iAmmoType );

	static CTFAmmoPack *Create( const Vector &vecOrigin, const QAngle &vecAngles, CBaseEntity *pOwner, const char *pszModelName, bool bDisintegrate = false );
	...
	void Disintegrate(bool bDisintegrate, CBaseEntity* pOwner);
	bool IsDisintegrating() { return m_bDisintegrate; }
	void SetExplosionOwner(CBaseEntity* pOwner) { m_pExplosionOwner = pOwner; }
...
private:
	...
	bool m_bDisintegrate;
	CBaseEntity *m_pExplosionOwner;
