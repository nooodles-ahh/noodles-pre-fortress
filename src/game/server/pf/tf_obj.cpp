...
void CBaseObject::InitializeMapPlacedObject( void )
{
	...
	{
		ConMsg("adding object to team %d!\n", GetTeamNumber());
...
bool CBaseObject::UpdateAttachmentPlacement( CBaseObject *pObject )
{
...
{
	// no ear piercing sound for map placed buildings
	if ( !m_iDefaultUpgrade )
		EmitSound( GetObjectInfo( ObjectType() )->m_pUpgradeSound );
}
...
void CBaseObject::StartPlacement( CTFPlayer *pPlayer )
{
	...
	// Changed for compatibility with TEAM_ANY and TEAM_UNASSIGNED
	// Set the skin
	switch( GetTeamNumber() )
	{
		case TF_TEAM_RED:
	...
	else
	{
		...
		for (int iTeam = FIRST_GAME_TEAM; iTeam < nTeamCount; ++iTeam)
		{
			...
			for (i = 0; i < pTeam->GetNumObjects(); i++)
			{
				...
				{
					if (FindNearestBuildPoint(pTempObject, pPlayer, flNearestPoint, vecNearestBuildPoint, false))
					{
						bSnappedToPoint = true;
						bShouldAttachToParent = true;
					}
...
int CBaseObject::OnTakeDamage( const CTakeDamageInfo &info )
{
	...
	// used to be npc_hurt but i don't want to touch this because it may break damage numbers
...
void CBaseObject::Killed( const CTakeDamageInfo &info )
{
	...
	if ( pScorer && pInflictor != this )
	{
		...
		if ( event )
		{
			event->SetInt( "userid", pTFPlayer ? pTFPlayer->GetUserID() : entindex() );
...
CTFTeam *CBaseObject::GetOpposingTFTeam( void )
{
...
#ifdef PF2_DLL
void CBaseObject::GetOpposingTFTeamList(CUtlVector<CTFTeam *> *pTeamList)
{
