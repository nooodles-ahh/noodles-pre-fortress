...
{
	IncrementStat( pPlayer, TFSTAT_ARMORREPAIRED, ( int )amount );
}
...
void CTFGameStats::Event_PlayerDamage( CBasePlayer *pBasePlayer, const CTakeDamageInfo &info, int iDamageTaken )
{
	...
	CBaseEntity *pAttacker = ToBasePlayer( info.GetAttacker() );
	...
	if (pAttacker != pSentry)
		IncrementStat( static_cast<CTFPlayer*>(pAttacker), TFSTAT_DAMAGE, iDamageTaken );
