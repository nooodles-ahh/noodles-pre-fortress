...
// wouldn't it be funny if we thought every tick
#define SENTRY_THINK_DELAY	TICK_INTERVAL 
...
#define SENTRYGUN_FRAG_RESIST_LVL_1		0.0
#define SENTRYGUN_FRAG_RESIST_LVL_2		0.2
#define SENTRYGUN_FRAG_RESIST_LVL_3		0.33


// DPN = Damage Per Nail
#define SENTRYGUN_NAILGUN_RESIST_LVL_1		0.0 // 10 DPN
#define SENTRYGUN_NAILGUN_RESIST_LVL_2		0.10 // 9 DPN
#define SENTRYGUN_NAILGUN_RESIST_LVL_3		0.20 // 8 DPN

#define SENTRYGUN_SAPPER_OWNER_DAMAGE_MODIFIER	0.33f

enum SentryAttachments
{	
...
void CObjectSentrygun::Spawn()
{
	...
	m_flNailgunResist = SENTRYGUN_NAILGUN_RESIST_LVL_1;
...
{	
	...
	// Based on "Smooth Sentry Construct & Upgrade Animations" plugin by Pelipoika
	// https://forums.alliedmods.net/showthread.php?p=2459381

	if( (m_iState == SENTRY_STATE_UPGRADING || m_iState == SENTRY_STATE_INACTIVE) && !IsUsingClientSideAnimation() )
	{
		RemoveAllGestures();
		// Cyanide; I wrote this line and it fixed the upgrade animation totally dying if the sentry was playing a gesture
		// Don't ask questions
		SetNumAnimOverlays( 0 );
		UseClientSideAnimation();
	}
	else if( m_iState != SENTRY_STATE_UPGRADING && m_iState != SENTRY_STATE_INACTIVE && IsUsingClientSideAnimation() )
	{
...
void CObjectSentrygun::Precache()
{
	...
	PrecacheScriptSound( "Building_Sentrygun.Idle1" );
...
void CObjectSentrygun::StartUpgrading( void )
{
	...
	case 2:
		...
		m_flNailgunResist = SENTRYGUN_NAILGUN_RESIST_LVL_2;
	...
	case 3:
		...
		m_flNailgunResist = SENTRYGUN_NAILGUN_RESIST_LVL_3;
...
bool CObjectSentrygun::FindTarget()
{
	...
	pPlayer = ToTFPlayer( GetOwner() );

	if ( !pPlayer )
		return false;

	pTeam = pPlayer->GetOpposingTFTeam();

	// TODO - kill everyone if there is no team
	if ( !pTeam )
		return false;
...
bool CObjectSentrygun::Fire()
{
	...
	if ( m_iAmmoShells > 0)
	{
		...
		info.m_iShots = 1;
		info.m_pAttacker = ( CBaseEntity* )GetBuilder();
...
void CObjectSentrygun::SentryRotate( void )
{
	...
	if ( !MoveTurret() )
	{
		...
		else
		{
			...
			default:
			case 1:
				EmitSound( "Building_Sentrygun.Idle1" );
				break;
			case 2:
...
int CObjectSentrygun::OnTakeDamage( const CTakeDamageInfo &info )
{
	...
	if ( GetBuilder() && info.GetAttacker()->GetTeamNumber() == GetBuilder()->GetTeamNumber() )
		return 0;
	...
	// As we increase in level, we get more resistant to Nails, to compensate for
	// our increased surface area taking more nailgun hits.
	CTFWeaponBase *pTFWeapon = dynamic_cast<CTFWeaponBase *>( info.GetInflictor() );
	if ( pTFWeapon && pTFWeapon->GetWeaponID() == TF_WEAPON_NAILGUN )
	{
		float flDamage = newInfo.GetDamage();

		flDamage *= ( 1.0 - m_flNailgunResist );
