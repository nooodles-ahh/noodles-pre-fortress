...
// Purpose: 
//-----------------------------------------------------------------------------
CTFMedicCallerPanel::CTFMedicCallerPanel( Panel *parent, const char *name, int iClass ) : EditablePanel(parent,name)
{
...
void CTFMedicCallerPanel::PerformLayout( void )
{
	...
	{
		pPanel = FindChildByName( "CallerBG" );
		if( pPanel )
		{
	...
	{
		bool bLowArmor = m_iType & ENGICALL_LOWARMOR;

		pPanel = FindChildByName( "EngiCallerBG" );
		if( pPanel )
		{
			pPanel->SetPos( (GetWide() - pPanel->GetWide()) * 0.5, (GetTall() - pPanel->GetTall()) * 0.5 );
			pPanel->SetVisible( !bLowArmor );
		}

		pPanel = FindChildByName( "EngiCallerLow" );
		if( pPanel )
		{
			pPanel->SetPos( (GetWide() - pPanel->GetWide()) * 0.5, (GetTall() - pPanel->GetTall()) * 0.5 );
			pPanel->SetVisible( bLowArmor );
		...
		pPanel = FindChildByName( "EngiCallerMelting" );
		if( pPanel )
		{
			pPanel->SetPos( (GetWide() - pPanel->GetWide()) * 0.5, (GetTall() - pPanel->GetTall()) * 0.5 );
			pPanel->SetVisible( m_iType & ENGICALL_MELTING );
		}
	}
}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CTFMedicCallerPanel::AddMedicCaller( C_TFPlayer *pPlayer, float flDuration, Vector &vecOffset, int iType /*= 0*/ )
{
	CTFMedicCallerPanel *pCaller = new CTFMedicCallerPanel( g_pClientMode->GetViewport(), "MedicCallerPanel", TF_CLASS_MEDIC );
...
// Purpose: 
//-----------------------------------------------------------------------------
void CTFMedicCallerPanel::AddEngiCaller( C_TFPlayer *pPlayer, float flDuration, Vector &vecOffset, int iType /*= 0*/ )
{
	CTFMedicCallerPanel *pCaller = new CTFMedicCallerPanel( g_pClientMode->GetViewport(), "MedicCallerPanel", TF_CLASS_ENGINEER );
	vgui::SETUP_PANEL( pCaller );
	pCaller->SetBounds( 0, 0, MEDICCALLER_WIDE, MEDICCALLER_TALL );
	pCaller->SetPlayer( pPlayer, flDuration, vecOffset );
	pCaller->SetType( iType );
