...
static const char* pszHoverupSound[TF_CLASS_MENU_BUTTONS] =
{
	0,
	"music.class_menu_01", // Scout
	"music.class_menu_02", // Sniper
	"music.class_menu_03", // Soldier
	"music.class_menu_04", // Demoman
	"music.class_menu_05", // Medic
	"music.class_menu_06", // Heavy
	"music.class_menu_07", // Pyro
	"music.class_menu_08", // Spy
	"music.class_menu_09", // Engineer
	"music.class_menu_10", // Civilian
	0,
	0
};
...
void CTFClassMenu::OnThink()
{
	...
	{
		if( m_pClassButtons[i] && m_pClassButtons[i]->IsWithin( mouseX, mouseY ) )
		{
			m_iCurrentClassIndex = i;
			break;
		}
	}
...
void CTFClassMenu::OnCommand( const char *command )
{
	...
	if( !Q_stricmp( command, "openloadout" ) )
	{
		engine->ClientCmd_Unrestricted( VarArgs("open_charinfo_direct %d", m_iCurrentClassIndex ) );
	}
	else
	{
