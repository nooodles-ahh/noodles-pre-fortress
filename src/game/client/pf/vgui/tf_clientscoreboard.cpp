...
void CTFClientScoreBoardDialog::ApplySchemeSettings( vgui::IScheme *pScheme )
{
	...
	if ( m_pImageList )
	{
		...
		{
			m_iImageClassIcons[i] = m_pImageList->AddImage( scheme()->GetImage( g_pszClassIcons[i], true ) );
			m_iImageClassIconsDead[i] = m_pImageList->AddImage( scheme()->GetImage( g_pszClassIcons[i+10], true ) );
			m_iImageClassIconsAlt[i] = m_pImageList->AddImage( scheme()->GetImage( g_pszClassIconsAlt[i], true ) );
			m_iImageClassIconsAltDead[i] = m_pImageList->AddImage( scheme()->GetImage( g_pszClassIconsAlt[i+10], true ) );
		}
...
void CTFClientScoreBoardDialog::UpdateArenaWaitingToPlayList( void )
{
	...
	for( int playerIndex = 1; playerIndex <= MAX_PLAYERS; playerIndex++ )
	{
		if( ShouldShowAsWaitingToPlay( playerIndex ) )
		{
			...
			if( TeamplayRoundBasedRules()->IsInArenaMode() && !tf_PR->GetArenaSpectator( playerIndex ) )
			{
		...
		if( g_PR->IsConnected( playerIndex ) )
		{
			...
			if ( !AreEnemyTeams( g_PR->GetTeam( playerIndex ), localteam ) && !( localteam == TEAM_UNASSIGNED ) )
			{
				...
				if( g_PR->IsConnected( playerIndex ) )
				{
					...
					bool bUseAltClassImages = tf_scoreboard_alt_class_icons.GetBool();
					if( iClass >= TF_FIRST_NORMAL_CLASS && iClass <= TF_LAST_NORMAL_CLASS )
					{
						if( tf_PR->IsAlive( playerIndex ) )
							pKeyValues->SetInt( "class", bUseAltClassImages ? m_iImageClassIconsAlt[iClass] : m_iImageClassIcons[iClass] );
						else
							pKeyValues->SetInt( "class", bUseAltClassImages ? m_iImageClassIconsAltDead[iClass] : m_iImageClassIconsDead[iClass] );
					}
					else
					{
						pKeyValues->SetInt( "class", NULL );
					}
				}				
			}
			else
...
void CTFClientScoreBoardDialog::UpdatePlayerDetails()
{
	...
	SetDialogVariable( "armorrepaired", roundStats.m_iStat[TFSTAT_ARMORREPAIRED] );
	SetDialogVariable( "playername", tf_PR->GetPlayerName( playerIndex ) );
