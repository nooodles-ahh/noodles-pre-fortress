...
bool CScriptObject::ReadFromBuffer( const char **pBuffer, bool isNewObject )
{
	...
	*pBuffer = engine->ParseFile( *pBuffer, token, sizeof( token ) );
	...
	// expect the next line to potentially be {, if not we have a tip
	if( strcmp( token, "{" ) )
	{
...
{
	pObjList = NULL;
	m_bLoadedDefault = false;
	m_flDefaultVersion = SCRIPT_VERSION;
}
...
bool CDescription::ReadFromBuffer( const char **pBuffer )
{
	...
	*pBuffer = engine->ParseFile( *pBuffer, token, sizeof( token ) );
	...
	if( !m_bLoadedDefault )
	{
		m_flDefaultVersion = fVer;
	}
	else if ( fVer != m_flDefaultVersion )
	{
		Msg( "Version mismatch, expecting %f, got %f", m_flDefaultVersion, fVer );
		return false;
	}
