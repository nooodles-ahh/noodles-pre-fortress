...
void UpdateStatSummaryPanels(CUtlVector<ClassStats_t> &param_1)
{
  FOR_EACH_VEC(g_vecStatPanels, i)
  {
	  g_vecStatPanels[i]->SetStats(param_1);
  }
}
...
// Purpose: Constructor
//-----------------------------------------------------------------------------
CTFStatsSummaryPanel::CTFStatsSummaryPanel(vgui::Panel *parent) : vgui::EditablePanel( parent, "TFStatsSummary", 
...
void CTFStatsSummaryPanel::UpdateClassDetails()
{
	...
	ClassDetails_t classDetails[] =
	{
		...
		{ TFSTAT_ARMORREPAIRED,			MAKEFLAG(TF_CLASS_ENGINEER),	"#TF_ClassRecord_MostArmorRepaired" },
	};
	...
	for ( ; iRow < ARRAYSIZE( classDetails ); iRow ++ )
	{
...
void CTFStatsSummaryPanel::UpdateControls()
{
	...
	m_pCloseButton->SetVisible( m_bInteractive && !m_bIsEmbedded );
#endif
}
...
void CTFStatsSummaryPanel::SetupForEmbedded()
{
	m_bIsEmbedded = true;
	UpdateDialog();
