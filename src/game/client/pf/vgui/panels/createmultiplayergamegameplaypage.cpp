...
{
	m_pOptionsList = new PanelListPanel( this, "GameOptions" );
...
void CCreateMultiplayerGameGameplayPage::ApplySchemeSettings( vgui::IScheme *pScheme )
{
	...
	if( !m_bLoaded )
	{
		m_pOptionsList->RemoveAll();
		m_pDescription->InitFromFile( GetDefaultFileName() );
		m_pDescription->InitFromFile( GetUserFileName() );
		m_pList = nullptr;

		LoadControlSettings( GetResName() );
		m_pOptionsList->SetFirstColumnWidth( 0 );
...
// Purpose: Constructor
//-----------------------------------------------------------------------------
CCreateMultiplayerGameModPage::CCreateMultiplayerGameModPage( vgui::Panel *parent, const char *name ) : 
	CCreateMultiplayerGameGameplayPage( parent, name )
{
}
...
void CCreateMultiplayerGameGameplayPage::OnApplyChanges()
{
	...
	if ( m_pDescription )
	{
		...
		fp = g_pFullFileSystem->Open( GetUserFileName(), "wb", "GAME");
		if ( fp )
		{
...
void CCreateMultiplayerGameGameplayPage::LoadGameOptionsList()
{
	...
	while ( pObj )
	{
		...
		m_pOptionsList->AddItem( nullptr, pCtrl );
...
//-----------------------------------------------------------------------------
CServerDescription::CServerDescription() : CDescription()
{
