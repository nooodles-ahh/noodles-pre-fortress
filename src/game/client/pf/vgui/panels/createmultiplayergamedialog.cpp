...
CCreateMultiplayerGameDialog *g_pCreateGameDialog = NULL;

CCreateMultiplayerGameDialog *OpenCreateGameDialog()
{
	if( NULL == g_pCreateGameDialog )
	{
		g_pCreateGameDialog = new CCreateMultiplayerGameDialog( nullptr );
	}
	return g_pCreateGameDialog;
}
...
CCreateMultiplayerGameDialog::CCreateMultiplayerGameDialog(vgui::Panel *parent) : PropertyDialog(parent, "CreateMultiplayerGameDialog")
{
	...
	m_pModPage = new CCreateMultiplayerGameModPage( this, "ModPage" );
	m_pBotPage = NULL;

	AddPage(m_pServerPage, "#GameUI_Server");
	AddPage(m_pGameplayPage, "#GameUI_Game");
	AddPage( m_pModPage, "#GameUI_Mod" );
...
void CCreateMultiplayerGameDialog::ShowDialog()
{
	SetVisible(true);
	MoveToFront();
...
CON_COMMAND( OpenCreateGameDialog, "Shows the create server dialog" )
{
	OpenCreateGameDialog()->ShowDialog();
}