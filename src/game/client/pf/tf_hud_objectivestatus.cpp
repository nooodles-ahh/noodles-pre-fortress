...
#include "pf_hud_multiflagstatus.h"
...
void CTFHudTimeStatus::SetTeamBackground( void )
{
	...
	if ( m_pTimePanelBG && TFGameRules() )
	{
		...
		const char *szImg = "../hud/objectives_timepanel_blue_bg";
...
void CTFHudTimeStatus::ApplySchemeSettings( IScheme *pScheme )
{
	...
	m_pTimePanelBG = dynamic_cast<vgui::ImagePanel*>( FindChildByName( "TimePanelBG" ) );
...
CTFHudObjectiveStatus::CTFHudObjectiveStatus( const char *pElementName ) : CHudElement( pElementName ), BaseClass( NULL, "HudObjectiveStatus" ) 
{
	...
	m_pMultiFlagPanel = new CPFHudMultiFlagObjectives( this, "ObjectiveMultiFlagPanel" );
...
void CTFHudObjectiveStatus::Reset()
{
	...
	if( m_pMultiFlagPanel )
	{
		m_pMultiFlagPanel->Reset();
	}
}
...
void CTFHudObjectiveStatus::SetVisiblePanels( void )
{
	...
	bool bMultiFlag = TFGameRules()->IsCTFFlagrun();

	// Hybrid CTFCP
	if( TFGameRules()->IsCTFCPHybrid() )
	{
	...
	switch ( TFGameRules()->GetGameType() )
	{
		case TF_GAMETYPE_CTF:
			...
			{
				// turn on the flag panel
				if( m_pMultiFlagPanel && !m_pMultiFlagPanel->IsVisible() )
				{
					m_pMultiFlagPanel->SetVisible( true );
				}
			}
			else
			{
...
void CTFHudObjectiveStatus::Think()
{
	...
	if ( m_pTimePanel )
	{
		...
		// Don't draw if we're in koth
		bool bKoth = TFGameRules()->IsInKothMode() && !TFGameRules()->IsInWaitingForPlayers();
		bool bDisplayTimer = !( pPlayer && pPlayer->GetObserverMode() == OBS_MODE_FREEZECAM ) && !bKoth;
		if ( bDisplayTimer )
		{
