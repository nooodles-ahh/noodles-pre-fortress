...
ConVar cl_draw_only_deathnotices( "cl_draw_only_deathnotices", "0", FCVAR_CHEAT );
...
{
	return ( CHudElement::ShouldDraw() && ( m_DeathNotices.Count() ) ) || (cl_draw_only_deathnotices.GetBool() && m_DeathNotices.Count());
}
...
void CTFHudDeathNotice::FireGameEvent( IGameEvent *event )
{
	...
	if ( FStrEq( pszEventName, "player_death" ) || FStrEq( pszEventName, "object_destroyed" ) )
	{
		...
		int iKillerTeam = g_PR->GetTeam( killer );
		m_DeathNotices[iMsg].Killer.iTeam = iKillerTeam;
		...
		if ( killedwith && *killedwith )
		{
			...
			{
				//this sets our crits backgorund
				//-Nbc66
				m_DeathNotices[iMsg].bCrit = true;
				if( iKillerTeam == TF_TEAM_BLUE )
					m_DeathNotices[iMsg].iconCritDeath = GetIcon( "d_crit_blue", bLocalPlayerInvolved );
				else
					m_DeathNotices[iMsg].iconCritDeath = GetIcon( "d_crit_red", bLocalPlayerInvolved );
		...
		{
			if ( !strcmp( m_DeathNotices[iMsg].szIcon, "d_worldspawn" ) || !strcmp( m_DeathNotices[iMsg].szIcon, "d_obj_srt" ) )
			{
	...
	else if ( FStrEq( "teamplay_flag_event", pszEventName ) )
	{
		...
		case TF_FLAGEVENT_DROPPED: 
			...
			break;
#ifdef PF2_CLIENT
		case TF_FLAGEVENT_THROWN:
			pszMsgKey = "#Msg_ThrownFlag";
			break;
#endif
		default:
	...
	else if ( FStrEq( "grenade_disarmed", pszEventName ) )
	{
		int iPlayerIndex = event->GetInt( "player" );
		int iDisarmerIndex = event->GetInt( "disarmer" );
		// only show to the players involved
		/*if ( iLocalPlayerIndex != iPlayerIndex && iLocalPlayerIndex != iDisarmerIndex)
		{
			m_DeathNotices.Remove( iMsg );
			return;
		}*/
		...
		// done!!!
		// -Nbc66
		wchar_t* localized = g_pVGuiLocalize->Find("#Msg_Disarmed");
		V_wcsncpy( m_DeathNotices[iMsg].wzInfoText, localized, sizeof( m_DeathNotices[iMsg].wzInfoText ) );

		const char *owner_name = g_PR->GetPlayerName( iPlayerIndex );
		Q_strncpy( m_DeathNotices[iMsg].Victim.szName, owner_name, ARRAYSIZE( m_DeathNotices[iMsg].Victim.szName ) );
		...
		const char *disarmer_name = g_PR->GetPlayerName( iDisarmerIndex );
		Q_strncpy( m_DeathNotices[iMsg].Killer.szName, disarmer_name, ARRAYSIZE( m_DeathNotices[iMsg].Killer.szName ) );
		m_DeathNotices[iMsg].Killer.iTeam = g_PR->GetTeam( iDisarmerIndex );

		if ( iLocalPlayerIndex == iPlayerIndex ||  iLocalPlayerIndex == iDisarmerIndex)
			m_DeathNotices[iMsg].bLocalPlayerInvolved = true;

		// print a log message
		Msg( "%s disarmed a grenade thrown by %s using %s\n", m_DeathNotices[iMsg].Killer.szName, m_DeathNotices[iMsg].Victim.szName, m_DeathNotices[iMsg].szIcon );
	}
...
void CTFHudDeathNotice::OnGameEvent( IGameEvent *event, int iDeathNoticeMsg )
{
	...
	if ( FStrEq( pszEventName, "player_death" ) || FStrEq( pszEventName, "object_destroyed" ) )
	{
		...
		case TF_DMG_CUSTOM_NAPALM_BURNING:
			...
			msg.wzInfoText[ 0 ] = 0;
			break;
		case TF_DMG_CUSTOM_INFECTION:
			Q_strncpy( msg.szIcon, "d_infection", ARRAYSIZE( msg.szIcon ) );
			msg.wzInfoText[ 0 ] = 0;
			break;
