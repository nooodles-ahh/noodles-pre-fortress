...
#define COLOR_PANEL_RED		"../hud/color_panel_red"
#define COLOR_PANEL_BLUE	"../hud/color_panel_blu"
#define COLOR_PANEL_BROWN	"../hud/color_panel_brown"
	...
	: EditablePanel( NULL, "FreezePanel" ), CHudElement( pElementName )
{
	...
	PrecacheMaterial( COLOR_PANEL_RED );
	PrecacheMaterial( COLOR_PANEL_BLUE );
	PrecacheMaterial( COLOR_PANEL_BROWN );
}
...
void CTFFreezePanel::FireGameEvent( IGameEvent * event )
{
	...
	else if ( Q_strcmp( "show_freezepanel", pEventName ) == 0 )
	{
		...
		if ( pKiller )
		{
			...
			bool bArmor = !pKiller->IsBaseObject() && pPlayer;
			if ( bArmor )
			{
				int iMaxArmor = pPlayer->GetPlayerClass()->GetMaxArmor();
				int iArmor = pPlayer->ArmorValue();
			...
			if ( pKiller->IsPlayer() )
			{
				...
				if( m_pFreezePanelBG )
				{
					if( pKiller->GetTeamNumber() == TF_TEAM_RED )
						m_pFreezePanelBG->SetImage( COLOR_PANEL_RED );
					else if( pKiller->GetTeamNumber() == TF_TEAM_BLUE )
						m_pFreezePanelBG->SetImage( COLOR_PANEL_BLUE );
					else
						m_pFreezePanelBG->SetImage( COLOR_PANEL_BROWN );
				}
			}
			else if (pKiller->IsBaseObject())
			{
				...
				if (pObj->GetType() != OBJ_SRT)
				{
					...
					if (pOwner)
					{
						...
						{
							m_pAvatar->SetShouldDrawFriendIcon(false);
							m_pAvatar->SetPlayer(pOwner, EAvatarSize::k_EAvatarSize64x64 );
						}

						pKiller = pOwner;
					}
				}
				else // spawnroom turret exception 
				{
					m_iKillerIndex = pObj->entindex();
					wchar_t *wszTurretLocalized = g_pVGuiLocalize->Find( "#TF_Object_SpawnroomTurret" );
					// TODO ADD TEAM TO KILLERNAME
					if ( wszTurretLocalized )
						m_pBasePanel->SetDialogVariable( "killername", wszTurretLocalized );

					bHealth = pObj->GetType() != OBJ_SRT/*pObj->m_takedamage > DAMAGE_EVENTS_ONLY*/;;
				...
				if ( m_pFreezeLabel )
				{
					// this is dumb, but I didn't want to add more if statements higher up
					// this is an exception for the spawnroom turret to use the normal killer localization
					// i would've put it higher up but theres player checking and shit and i don't
					// want to complicate the code even more despite me kinda already doing that here...
					// this appends object to the end of the string in settext here
					// if the killer is the spawnroom turret, the string will be null and use the 
					// normal killer text, else use the object localization as normal.
					const char* object = (pObj->GetType() == OBJ_SRT) ? "\0" : "Object";
					if( pKiller && !pKiller->IsAlive() )
					{
						m_pFreezeLabel->SetText( CFmtStr("#FreezePanel_Killer%s_Dead", object ) );
					}
					else
					{
						m_pFreezeLabel->SetText( CFmtStr( "#FreezePanel_Killer%s", object ) );
					}
				}
#else
				if( m_pFreezeLabel )
			...
			m_pKillerArmor->SetVisible( bArmor );
			m_pKillerHealth->SetVisible( bHealth );

			m_pFreezeLabel->SetVisible( true );
			m_pFreezeLabelKiller->SetVisible( true );
			m_pAvatar->SetVisible( true );

			if( !bHealth )
			{
				m_pFreezeLabel->SetPos( m_nNoHPLabelXPos, m_pFreezeLabel->GetYPos() );
				m_pFreezeLabelKiller->SetPos( m_nNoHPKillerXPos, m_pFreezeLabelKiller->GetYPos() );
				m_pAvatar->SetPos( m_nNoHPAvatarXPos, m_pAvatar->GetYPos() );
			}
			else
			{
				if( !bArmor )
				{
					m_pFreezeLabel->SetPos( m_nDefaultLabelXPos, m_pFreezeLabel->GetYPos() );
					m_pFreezeLabelKiller->SetPos( m_nDefaultKillerXPos, m_pFreezeLabelKiller->GetYPos() );
					m_pAvatar->SetPos( m_nDefaultAvatarXPos, m_pAvatar->GetYPos() );
				}
				else
				{
					m_pFreezeLabel->SetPos( m_nArmorLabelXPos, m_pFreezeLabel->GetYPos() );
					m_pFreezeLabelKiller->SetPos( m_nArmorKillerXPos, m_pFreezeLabelKiller->GetYPos() );
					m_pAvatar->SetPos( m_nArmorAvatarXPos, m_pAvatar->GetYPos() );
				}
			}
